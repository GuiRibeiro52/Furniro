    ...
    <div className="container mx-auto py-10 font-poppins">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {visibleProducts.map(product => (
          <div key={product.id} className="w-[285px] h-[500px] bg-quartiary relative group">
            <img src={`${product.images.mainImage}`} alt={product.title} className="w-full h-[200px] object-cover mb-4" />
            <h3 className="text-xl font-semibold ml-4 mt-4 mb-2">{product.title}</h3>
            <p className="text-sm text-tertiary ml-4 mb-2">{product.description.short}</p>
            <p className="font-semibold text-xl ml-4">R$ {product.salePrice}</p>
